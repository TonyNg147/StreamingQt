...
CameraStorage::CameraStorage(QObject* parent)
	...
	,m_cameraService{new CameraStorageService{videoDuration, NumberOfFile, "C:\\Users\\Tony\\Desktop\\hehe\\SavingVideoFolder\\Nguyen"}}
{
	m_cameraService->setObservant(this);
	QObject::connect(m_recorder.get(), &QMediaRecorder::errorOccurred, this, [this](QMediaRecorder::Error error, const QString &errorString){
		qWarning() << "error is " << error << " " << errorString;
	});

	QObject::connect(m_cameraService.get(), &CameraStorageService::serviceTrackingFinished, this, [this](){
		m_recorder->stop();
	});
}
