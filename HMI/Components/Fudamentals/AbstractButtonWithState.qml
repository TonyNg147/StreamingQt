		...
		if (behavior === ComponentEnum.IntimeLoadItem) {
			...
			for (let attr of attributeOfButtons) {
				buttonAttrs.append({
					...
					"buttonState": attr.buttonState
				});
			}
			inTimeLoadItemsLoader.active = true;
		} else if (scripts.toStateON === undefined && scripts.toStateOFF === undefined && scripts.toStateINACTIVE === undefined && scripts.toStateACTIVE === undefined) {
